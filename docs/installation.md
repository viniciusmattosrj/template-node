...
Faça um git clone do projeto template-node:
```
git@github.com:viniciusmattosrj/template-node.git
```
...
No projeto **template-node** execute o comando abaixo:
```
yarn install
yarn dev
```
...
Dentro do projeto **template-node** execute:
```
docker-compose up -d
```
